...
public class MovementController {
    ...
    public MovementController(AlertClient alertClient) {
    ...
    public SensorEventResponse ingest(@RequestBody SensorEventRequest request) {
        boolean critical = request.value() >= 1.0;
        ...
        if (critical) {
            alertClient.sendCriticalAlert(request.source(), request.value(), "Movement detected");
        }

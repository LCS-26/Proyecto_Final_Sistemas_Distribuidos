...
/**
 * Bean con ámbito Prototype para construir el contexto de eventos de acceso.
 *
 ...
 * - @PreDestroy: limpieza antes de destrucción del bean (gestionado por caller en Prototype)
 */
@Component
...
public class AlertEventBuilder {
    ...
    @PostConstruct
    public void init() {
        LOGGER.debug("event=alert_builder_created source=access-service " +
                "message='Nueva instancia de AlertEventBuilder creada (Prototype scope)'");
    }

    @PreDestroy
    public void destroy() {
        LOGGER.debug("event=alert_builder_destroyed source=access-service " +
